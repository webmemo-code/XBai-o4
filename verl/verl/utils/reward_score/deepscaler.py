...
class RewardMathFn(RewardFn):
    ...
    def __call__(self, input: RewardInput) -> RewardOutput:
        ...
        if THOUGHT_DELIMITER_START in model_response and THOUGHT_DELIMITER_END in model_response:
            ...
            return RewardOutput(reward=-2, is_correct=False)
...
def deepscaler_reward_fn(solution_str: str, ground_truth: Union[str, List[str]], enable_llm = False):
    ...
    return reward_response.is_correct, reward_response.reward


def find_invalid_confuse_tags(text):
    # 匹配所有 <confuse> 和 </confuse> 位置
    open_tags = [m.start() for m in re.finditer(r'<confuse>', text)]
    close_tags = [m.start() for m in re.finditer(r'</confuse>', text)]

    # 如果开标签和闭标签数量不相等，说明有不完整的标签
    if len(open_tags) != len(close_tags):
        return True

    # 检查匹配顺序，确保 <confuse> 先出现，然后 </confuse>
    for open_pos, close_pos in zip(open_tags, close_tags):
        if open_pos > close_pos:  # 如果出现 </confuse> 早于 <confuse>
            return True
    
    return False


def confuse_reward(str):
    pattern = r'<critical>'
    match = re.findall(pattern, str, re.DOTALL | re.MULTILINE)
...
def compute_score(solution_str, ground_truth, extra_info):
    ...
    train_flag = 1.0 if extra_info['split']=='train' else 0.0
    # confuse_num = confuse_reward(solution_str)

    is_correct, reward = deepscaler_reward_fn(solution_str, ground_truth)
    
    if is_correct:
        if train_flag:
            return 1.0 #if confuse_num>0 else 0.9
        else:
            return 1.0
    else:
        if train_flag:
            return 0.1 if reward==-2 else 0.0 #if confuse_num>0 else -0.1
        else: 
            return 0.0
