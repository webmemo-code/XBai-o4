...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_score_model_optimizer(self,
                                     model_config,
                                     fsdp_config,
                                     optim_config,
                                     role='actor'):
        ...
        c = model_config.channel
        score_module = nn.Sequential(
            nn.Linear(c, c*2),
            nn.ReLU(),
            nn.Dropout(p=0.5),
            nn.Linear(c*2, 1)
        ...
        if model_config.model_path is not None:
            score_model_state_dict = torch.load(model_config.model_path, map_location='cpu')
            score_module.load_state_dict(score_model_state_dict)
            print(f'Load score mdoel from {model_config.model_path}')
        score_module.to(torch_dtype)
        ...
        auto_wrap_policy = get_fsdp_wrap_policy(module=score_module, config=fsdp_config.get('wrap_policy', None))
        ...
        if fsdp_strategy == "fsdp":
            ...
            full_state = score_module.state_dict()
            apply_fsdp2(score_module, fsdp_kwargs, fsdp_config)
            fsdp2_load_full_state_dict(score_module, full_state, fsdp_mesh, cpu_offload)
            score_module_fsdp = score_module
        else:
        ...
        if role == "actor" and optim_config is not None:
            ...
            score_optimizer = optim_fun(
                score_module_fsdp.parameters(),
                lr=optim_config.lr,
            ...
            if self.config.score.optim.num_warmup_steps:
                print(f'Edit warmup steps...')
                num_warmup_steps = self.config.score.optim.num_warmup_steps

            print(f"Total steps: {total_steps}, num_warmup_steps: {num_warmup_steps}")

            if warmup_style == "constant":
                score_lr_scheduler = get_constant_schedule_with_warmup(optimizer=score_optimizer, num_warmup_steps=num_warmup_steps)
            elif warmup_style == "cosine":
                score_lr_scheduler = get_cosine_schedule_with_warmup(optimizer=score_optimizer, num_warmup_steps=num_warmup_steps, num_training_steps=total_steps, min_lr_ratio=min_lr_ratio, num_cycles=num_cycles)
            else:
        ...
        return score_module_fsdp, score_optimizer, score_lr_scheduler
    ...
    def init_model(self):
        ...
        if self._is_actor:
            if self.config.get('use_score', False):
                self.score_module_fsdp, self.score_optimizer, self.score_lr_scheduler = self._build_score_model_optimizer(
                    model_config=self.config.score,
                    fsdp_config=fsdp_config,
                    optim_config=self.config.score.optim,
                )
                if fsdp_version(self.score_module_fsdp) == 1:
                    self.score_module = self.score_module_fsdp._fsdp_wrapped_module

                if self._is_offload_optimizer:
                    offload_fsdp_optimizer(optimizer=self.score_optimizer)
            else:
                self.score_module_fsdp, self.score_optimizer, self.score_lr_scheduler = None, None, None
            ...
            self.actor = DataParallelPPOActor(config=self.config.actor,
                                  ...
                                  score_module=self.score_module_fsdp,
                                  score_optimizer=self.score_optimizer,
                                  tokenizer=self.tokenizer)
    ...
    def update_actor(self, data: DataProto):
        ...
        if self._is_offload_param:
            ...
            if self.config.get('use_score', False):
                load_fsdp_model_to_gpu(self.score_module_fsdp)
        if self._is_offload_optimizer:
            load_fsdp_optimizer(optimizer=self.actor_optimizer, device_id=torch.cuda.current_device())
            if self.config.get('use_score', False):
                load_fsdp_optimizer(optimizer=self.score_optimizer, device_id=torch.cuda.current_device())
        ...
        with self.ulysses_sharding_manager:
            ...
            if self.config.get('use_score', False):
                ...
                score_lr = self.score_lr_scheduler.get_last_lr()[0]
                metrics['actor/score_lr'] = score_lr
        ...
        if self._is_offload_param:
            offload_fsdp_model_to_cpu(self.actor_module_fsdp)
            if self.config.get('use_score', False):
                offload_fsdp_model_to_cpu(self.score_module_fsdp)
        if self._is_offload_optimizer:
            offload_fsdp_optimizer(optimizer=self.actor_optimizer)
            if self.config.get('use_score', False):
                offload_fsdp_optimizer(optimizer=self.score_optimizer)
    ...
    def save_checkpoint(self, local_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None):
        ...
        if self.config.get('use_score', False):
            ...
            with FSDP.state_dict_type(self.score_module_fsdp, StateDictType.FULL_STATE_DICT, save_policy):
                cpu_state = self.score_module_fsdp.state_dict()
            score_model_path = os.path.join(local_path, 'score_module.pt')
            if self.rank == 0:
                torch.save(cpu_state, score_model_path)
    ...
    def load_checkpoint(self, local_path, hdfs_path=None, del_local_after_load=False):
        ...
        no_load_optim = False
        if self.config.actor.optim.no_load_optim:
            ## requires verification 
            print('No load actor optim...')
            no_load_optim = True

        self.checkpoint_manager.load_checkpoint(local_path=local_path, hdfs_path=hdfs_path, del_local_after_load=del_local_after_load, no_load_optim=no_load_optim)
        ...
        if self.config.get('use_score', False):
            ...
            if os.path.exists(score_model_path):
                score_model_state_dict = torch.load(score_model_path, map_location='cpu')
                self.score_module_fsdp.load_state_dict(score_model_state_dict)
                print(f'[rank-{self.rank}]: Loading from {score_model_path}')
            else:
...
class CriticWorker(Worker):
    ...
    def _build_critic_model_optimizer(self, config):
        ...
        critic_optimizer = optim_fun(critic_module.parameters(),
                                       lr=config.optim.lr,
