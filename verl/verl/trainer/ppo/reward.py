...
def compute_reward(data: DataProto, reward_fn, global_steps):
    """
    ...
    try:
        ...
        reward_tensor = reward_fn(data, global_steps)
...
def compute_reward_async(data: DataProto, config, tokenizer, global_steps):
    """
    Load the reward manager and compute the reward for a batch of data.
    This is meant to be run in a separate Ray worker.
    """
    reward_fn = load_reward_manager(config, tokenizer, num_examine=0, **config.reward_model.get("reward_kwargs", {}))
    return compute_reward(data, reward_fn, global_steps)
