...
=======
## 通用reward配置
参考./examples/grpo_trainer/run_qwen2-7b_stage1_math_4node_128k_general.sh
>>>>>>> dev_xmt

config.trainer.use_model_reward=True，且训练数据中data_source标签设置为general，支持单实例triton服务model_reward计算

**triton服务配置**
需要在./verl/utils/reward_score/model_reward/__init__.py中指定服务的url及输入的拼接格式，目前默认按照tokenizer.apply_chat_template格式拼接

## 学习率配置
config.actor_rollout_ref.actor.optim.warmup_style=cosine
...
=======
config.actor_rollout_ref.actor.optim.min_lr_ratio=最终衰减到初始学习率的比例

## 断点续训配置
参考./examples/grpo_trainer/run_qwen2-7b_stage1_math_4node_32k.sh
>>>>>>> dev_xmt
...
=======
config.data.resume_global_steps=断点的global_step

config.trainer.resume_mode=需要加载的断点ckpt路径
>>>>>>> dev_xmt

config.trainer.resume_from_path=True
